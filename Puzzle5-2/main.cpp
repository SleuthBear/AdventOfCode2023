    ...
    for(int i = 0; i < line.length(); i++) {
        if('0' <= line[i] and line[i] <= '9') {
            ...
            if(sourceNum == true) {
                ...
                for(int j = 0; j <= val; j++) {
                    numbers.push_back(currentSource + j);
                }
                sourceNum = true;
            }
            val = 0;
...
int main()
{
    ...
    std::ifstream myFile("/Users/dylanbeaumont/CLionProjects/AdventOfCode/Puzzle5-2/input.txt");
    if (!myFile) {
    ...
    std::cout << "toMaps: " << toMaps[0] << std::endl;
